...
_IMG_DIR = os.path.join(_BASE_DIR, 'img1')

datasets = {
    'kodak':  dnnlib.EasyDict(dataset_dir=_IMG_DIR),  # Use training images directly
    'bsd300': dnnlib.EasyDict(dataset_dir=os.path.join(_BASE_DIR, 'datasets/bsd300')),
    'set14':  dnnlib.EasyDict(dataset_dir=os.path.join(_BASE_DIR, 'datasets/set14')),
    'train':  dnnlib.EasyDict(dataset_dir=_IMG_DIR)
}

def ensure_dataset_exists(dataset_name, dataset_config):
    """Ensure dataset directory exists and copy training images if needed."""
    abs_path = os.path.abspath(dataset_config.dataset_dir)
    train_path = os.path.join(_BASE_DIR, 'img1')  # Use the training images directory
    
    if not os.path.exists(abs_path):
        os.makedirs(abs_path, exist_ok=True)
        print(f"Created dataset directory {abs_path}")
        
        # Copy training images to validation directory
        if os.path.exists(train_path):
            print(f"Copying training images to validation directory...")
            for f in os.listdir(train_path):
                if f.endswith(('.png', '.jpg', '.jpeg', '.JPEG', '.PNG')):
                    src = os.path.join(train_path, f)
                    dst = os.path.join(abs_path, f)
                    shutil.copy2(src, dst)
            print(f"Copied images to {abs_path}")
            return dataset_config
        else:
            print(f"Warning: Training directory {train_path} not found")
            return dnnlib.EasyDict(dataset_dir=train_path)
    return dataset_config
...
if __name__ == "__main__":
    def train(args):
        ...
        if 'train_tfrecords' in args and args.train_tfrecords is not None:
            # Use absolute paths
            train_config.train_tfrecords = os.path.join(_BASE_DIR, args.train_tfrecords)
            train_dir = os.path.dirname(train_config.train_tfrecords)
            os.makedirs(train_dir, exist_ok=True)
            
            # Use training images directory directly for validation
            train_config.validation_config = dnnlib.EasyDict(dataset_dir=_IMG_DIR)
            
            if not os.path.exists(train_config.train_tfrecords):
                print(f'ERROR: TFRecords file not found: {train_config.train_tfrecords}')
                sys.exit(1)
